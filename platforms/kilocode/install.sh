#!/bin/bash
#
# Install discuss-for-specs skill for Kilocode (L1 platform)
#
# Kilocode Skills Directory Structure:
...
INSTALL_DIR="${KILOCODE_SKILLS_DIR:-$HOME/.kilocode/skills}"

# Parse arguments
while [[ $# -gt 0 ]]; do
    case $1 in
        --project)
            INSTALL_DIR=".kilocode/skills"
            shift
            ;;
        --dir)
            INSTALL_DIR="$2"
        ...
        *)
            ...
            echo ""
            echo "Options:"
            echo "  --project    Install to project directory (.kilocode/skills/)"
            echo "  --dir <dir>  Install to custom directory"
            echo ""
            echo "Default: Install to ~/.kilocode/skills/"
            exit 1
            ;;
    esac
done
...
echo ""
echo "To use: Say \"讨论一下\" or \"let's discuss\" in Kilocode"
